        ...
        <EditableSection sectionName="personalInfo" isEditing={isEditing} onClick={onSectionClick} isActive={activeSection === "personalInfo"} onRegenerate={onRegenerate} isRegenerating={isRegenerating}>
          <div className="mb-2 border-b-2 border-blue-500 pb-6">
            ...
            <div className="flex flex-col gap-3 text-sm text-slate-300">
              <PrintLink className="flex items-center gap-2 hover:text-white transition-colors" isEditing={isEditing} href={resumeData.personalInfo.email ? `mailto:${resumeData.personalInfo.email}` : ""}><span className="text-blue-400">✉</span> <InlineEdit value={resumeData.personalInfo.email} isEditing={isEditing} onChange={(v) => onUpdateSection('personalInfo', { ...resumeData.personalInfo, email: v })} placeholder="Email" /></PrintLink>
              <PrintLink className="flex items-center gap-2 hover:text-white transition-colors" isEditing={isEditing} href={resumeData.personalInfo.phone ? `tel:${resumeData.personalInfo.phone}` : ""}><span className="text-blue-400">☎</span> <InlineEdit value={resumeData.personalInfo.phone} isEditing={isEditing} onChange={(v) => onUpdateSection('personalInfo', { ...resumeData.personalInfo, phone: v })} placeholder="Phone" /></PrintLink>
            ...
            <div className="mt-5 flex flex-col gap-2 text-sm text-slate-400">
              ...
              {extraLinks.map((link, i) => {
                ...
                return (isEditing || link.url?.trim()) && (
                  <PrintLink key={i} className="flex items-center gap-1.5 hover:text-white transition-colors" isEditing={isEditing} href={link.url}>
                    <SocialIconToggle visible={true} onToggle={() => onUpdateSection('personalInfo', { ...resumeData.personalInfo, extraLinks: extraLinks.filter((_, idx) => idx !== i) })} isEditing={isEditing}><ExtraIcon size={11} className="shrink-0 text-blue-400" /></SocialIconToggle>
                    {isEditing ? (
                      <>
                        ...
                        <span className="text-blue-400"><InlineEdit value={link.url} isEditing={true} onChange={(v) => onUpdateSection('personalInfo', { ...resumeData.personalInfo, extraLinks: extraLinks.map((l, idx) => idx === i ? { ...l, url: v } : l) })} placeholder="URL" /></span>
                      </>
                    ) : (
                      <span className="text-blue-400">{link.label?.trim() || link.url}</span>
                    )}
                  </PrintLink>
                );
              })}
              {isEditing && (
                <button type="button" onClick={(e) => { e.stopPropagation(); onUpdateSection('personalInfo', { ...resumeData.personalInfo, extraLinks: [...extraLinks, { label: "Link", url: "" }] }); }} className="text-blue-400 opacity-50 hover:opacity-100 transition-opacity uppercase text-[10px] font-bold tracking-wider mt-1">+ Add Link</button>
              )}
            </div>
          </div>
        </EditableSection>
