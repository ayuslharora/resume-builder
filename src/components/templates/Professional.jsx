          ...
          <div className="mb-8">
            ...
            <div className="mt-4 text-sm flex flex-col gap-2 text-gray-300">
              {(isEditing || resumeData.personalInfo.linkedin) && (
                <PrintLink className="flex items-center gap-1.5 hover:text-white" isEditing={isEditing} href={resumeData.personalInfo.linkedin}>
                  ...
                  {isEditing ? (<><InlineEdit value={resumeData.labels?.linkedin ?? "LinkedIn:"} isEditing={true} onChange={(v) => onUpdateSection('labels', { ...resumeData.labels, linkedin: v })} /> <InlineEdit value={resumeData.personalInfo.linkedin} isEditing={true} onChange={(v) => onUpdateSection('personalInfo', { ...resumeData.personalInfo, linkedin: v })} placeholder="URL" /></>) : (<span>{(resumeData.labels?.linkedin ?? 'LinkedIn').replace(/:+\s*$/, '').trim()}</span>)}
                </PrintLink>
              )}
              {(isEditing || resumeData.personalInfo.github) && (
                <PrintLink className="flex items-center gap-1.5 hover:text-white" isEditing={isEditing} href={resumeData.personalInfo.github}>
                  ...
                  {isEditing ? (<><InlineEdit value={resumeData.labels?.github ?? "GitHub:"} isEditing={true} onChange={(v) => onUpdateSection('labels', { ...resumeData.labels, github: v })} /> <InlineEdit value={resumeData.personalInfo.github} isEditing={true} onChange={(v) => onUpdateSection('personalInfo', { ...resumeData.personalInfo, github: v })} placeholder="URL" /></>) : (<span>{(resumeData.labels?.github ?? 'GitHub').replace(/:+\s*$/, '').trim()}</span>)}
                </PrintLink>
              )}
              {extraLinks.map((link, i) => {
                const ExtraIcon = getIconForUrl(link.url);
                return (isEditing || link.url?.trim()) && (
                  <PrintLink key={i} className="flex items-center gap-1.5 hover:text-white" isEditing={isEditing} href={link.url}>
                    ...
                    {isEditing ? (
                      <>
                        ...
                        <InlineEdit value={link.url} isEditing={true} onChange={(v) => onUpdateSection('personalInfo', { ...resumeData.personalInfo, extraLinks: extraLinks.map((l, idx) => idx === i ? { ...l, url: v } : l) })} placeholder="URL" />
                      </>
                    ) : (
                      <span>{link.label?.trim() || link.url}</span>
                    )}
                  </PrintLink>
                );
              })}
              {isEditing && (
                <button type="button" onClick={(e) => { e.stopPropagation(); onUpdateSection('personalInfo', { ...resumeData.personalInfo, extraLinks: [...extraLinks, { label: "Link", url: "" }] }); }} className="opacity-50 hover:opacity-100 transition-opacity uppercase text-[10px] font-bold tracking-wider mt-1">+ Add Link</button>
              )}
            </div>
          </div>
        </EditableSection>
                  ...
                  <div>
                    ...
                    <ul className="list-disc list-inside space-y-1">
                      {(resumeData.skills.technical || []).map((s, i) => (isEditing || s?.toString()?.trim()) && (
                        <li key={i} className="relative group/skill"><InlineEdit value={s} isEditing={isEditing} onChange={(v) => onUpdateSection('skills', { ...resumeData.skills, technical: Object.assign([...resumeData.skills.technical], {[i]: v}) })} />{isEditing && <button type="button" contentEditable={false} onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); onUpdateSection('skills', { ...resumeData.skills, technical: resumeData.skills.technical.filter((_, idx) => idx !== i) }); }} className="absolute -top-1 -right-1 w-3.5 h-3.5 rounded-full bg-red-500 text-white hidden group-hover/skill:flex items-center justify-center text-[9px] leading-none z-10 cursor-pointer" title="Remove">×</button>}</li>
                      ))}
                      {isEditing && (
                        <li>
                          <button
                  ...
                  <div>
                    ...
                    <ul className="list-disc list-inside space-y-1">
                      {(resumeData.skills.soft || []).map((s, i) => (isEditing || s?.toString()?.trim()) && (
                        <li key={i} className="relative group/skill"><InlineEdit value={s} isEditing={isEditing} onChange={(v) => onUpdateSection('skills', { ...resumeData.skills, soft: Object.assign([...resumeData.skills.soft], {[i]: v}) })} />{isEditing && <button type="button" contentEditable={false} onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); onUpdateSection('skills', { ...resumeData.skills, soft: resumeData.skills.soft.filter((_, idx) => idx !== i) }); }} className="absolute -top-1 -right-1 w-3.5 h-3.5 rounded-full bg-red-500 text-white hidden group-hover/skill:flex items-center justify-center text-[9px] leading-none z-10 cursor-pointer" title="Remove">×</button>}</li>
                      ))}
                      {isEditing && (
                        <li>
                          <button
