...
export function buildHomepageStaticHtml() {
  ...
  const faqMarkup = HOME_SEO_FAQ.map(
    (item) => `
      <div class="space-y-1">
        <dt class="text-sm font-medium text-on-surface">${escapeHtml(item.question)}</dt>
        <dd class="text-sm leading-relaxed text-on-surface-variant">${escapeHtml(item.answer)}</dd>
      </div>
    `,
  ).join('')
  ...
  return `
    <div id="homepage-static-shell" class="landing-desktop-shell min-h-screen px-6 pt-6 pb-24 fade-in relative overflow-hidden">
      ...
      <div class="landing-hero-grid w-full max-w-7xl relative z-10">
        <section class="w-full space-y-5 text-center flex flex-col items-center">
          ...
          <div class="flex items-center justify-center gap-5 pt-1 flex-wrap">
            ${trustItems}
          </div>
        </section>

        <aside class="landing-feature-panel w-full max-w-[440px] rounded-[1.5rem] p-5.5 xl:p-6 relative glass-strong">
      ...
      <section class="landing-seo-grid relative z-10 mt-8 w-full max-w-7xl rounded-[1.5rem] p-4 sm:p-5 glass" aria-labelledby="landing-seo-heading-static">
        ...
        <div class="mt-4 rounded-2xl border border-white/[0.06] bg-white/[0.03] p-4">
          <h2 class="text-base font-semibold text-on-surface">FAQ</h2>
          <dl class="mt-3 space-y-3">${faqMarkup}</dl>
        </div>
      </section>
